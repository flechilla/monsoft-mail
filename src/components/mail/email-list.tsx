...
const categoryColors: Record<string, string> = {
  primary: 'bg-blue-500/8 text-blue-400 border-blue-500/15',
  updates: 'bg-emerald-500/8 text-emerald-400 border-emerald-500/15',
  promotions: 'bg-amber-500/8 text-amber-400 border-amber-500/15',
  social: 'bg-purple-500/8 text-purple-400 border-purple-500/15',
  forums: 'bg-orange-500/8 text-orange-400 border-orange-500/15',
};
  ...
  if (emails.length === 0) {
    return (
      <div className="flex h-full flex-col items-center justify-center gap-3 text-muted-foreground">
        <Mail className="h-10 w-10 opacity-15" />
        <span className="text-sm text-muted-foreground/60">No emails</span>
      </div>
    );
  }
      ...
      {emails.map((email) => {
        ...
        return (
          <button
            ...
            className={`email-row group flex w-full items-center gap-3 rounded-lg px-3 py-2 text-left transition-all duration-200 ${
              isSelected
                ? 'nav-active bg-primary/6 shadow-sm'
                : 'hover:bg-muted/30 hover:translate-x-0.5'
            }`}
          >
            {/* Unread indicator */}
            <div className="w-1.5 self-stretch shrink-0 flex items-center">
              {isUnread && <div className="h-1.5 w-1.5 rounded-full bg-primary shadow-[0_0_6px_rgba(59,130,246,0.4)]" />}
            </div>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                ...
                <div className="flex shrink-0 items-center gap-1.5 ml-auto">
                  ...
                  {/* Time */}
                  <span className="hide-on-hover font-mono text-[11px] text-muted-foreground/50 whitespace-nowrap tabular-nums tracking-tight">
                    {formatDistanceToNow(dateStr)}
                  </span>
                </div>
              </div>
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`truncate text-[13px] leading-tight ${isUnread ? 'font-medium text-foreground' : 'text-muted-foreground'}`}>
                  {email.subject}
                </span>
              </div>
              <div className="flex items-center gap-2 mt-0.5">
                {email.snippet && (
                  <p className="truncate text-[12px] leading-relaxed text-muted-foreground/40 flex-1">
                    {email.snippet}
                  </p>
                )}
                <div className="flex shrink-0 items-center gap-1">
                  {category && categoryColors[category] && (
                    <span
                      className={`rounded-full border px-2 py-0 text-[10px] font-medium tracking-tight ${categoryColors[category]}`}
                    >
                      {email.aiCategory}
                    </span>
                  )}
                  {email.aiPriority != null && email.aiPriority <= 2 && (
                    <span className="rounded-full border border-red-500/15 bg-red-500/8 px-2 py-0 text-[10px] font-medium text-red-400">
                      P{email.aiPriority}
                    </span>
                  )}
                </div>
